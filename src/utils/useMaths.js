...
function MathsProvider({ children }) {
    ...
    useEffect(() => {
        const [n1, n2, d1, d2] = [ Math.floor(Math.random() * seed + 0),  Math.floor(Math.random() * seed + 1),  
        Math.floor(Math.random() * seed + 2),  Math.floor(Math.random() * seed + 3) ]
    ...
    const value = {setCalculation, calculation, numbers, dummyAnswers, answer, setAnswer, 
        getNewCalculation: () => setTrigger(!trigger), setDifficulty: (difficulty) => setSeed(difficulty),
        difficulty: seed};
